...
status_t BufferQueueProducer::queueBuffer(int slot,
    ...
    { // Autolock scope
        ...
        // MStar Android Patch Begin
#ifdef RENDER_IN_QUEUE_BUFFER
        if ((graphicBuffer->getUsage() & GRALLOC_USAGE_PRIVATE_2) && (mCore->mConsumerName == "SurfaceView")) {
            void *vaddr = NULL;
            ...
            if (vaddr != NULL) {
                ...
                mOverlayID = dff1->OverlayID;
                MS_Video_Disp_Flip(MS_Win_ID(dff1->OverlayID), (MS_DispFrameFormat*)vaddr);
                #ifdef BLOCK_UNTIL_DISP_INIT_DONE
                memcpy(&dff, vaddr, sizeof(MS_DispFrameFormat));
                #endif
                if (!dff.u8FreezeThisFrame
                    && (MS_Video_Disp_Get_State((MS_Win_ID)dff.OverlayID) == MS_STATE_OPENED)
                    && dff.u8CurIndex >= MAX_VSYNC_BRIDGE_DISPQ_NUM){
                    BQ_LOGD("BufferQueue Drop");
                ...
                BQ_LOGE("queueBuffer: graphicBuffer is NULL");
            }
    ...
    // MStar Android Patch Begin
#if defined(RENDER_IN_QUEUE_BUFFER) && defined(BLOCK_UNTIL_DISP_INIT_DONE)
    if ((mCore->mConsumerName == "SurfaceView") && (dff.OverlayID != 0xFFFF)) {
        if (!MS_Video_Disp_Wait(MS_Win_ID(dff.OverlayID), MS_WAIT_INIT_DONE, &dff)
            && !dff.u8IsNoWaiting) {
            Mutex::Autolock lock(mCore->mMutex);
            const sp<GraphicBuffer>& graphicBuffer(mSlots[slot].mGraphicBuffer);
            if (graphicBuffer != NULL) {
                ...
                status_t err = graphicBuffer->lock(GRALLOC_USAGE_SW_READ_MASK, (void**)(&vaddr));
                if (vaddr != NULL) {
                    BQ_LOGD("wait disp path init done");
                    MS_Video_Disp_Flip(MS_Win_ID(dff.OverlayID), (MS_DispFrameFormat*)vaddr);
                    graphicBuffer->unlock();
                }
            }
        }
...
int BufferQueueProducer::query(int what, int *outValue) {
    ...
    switch (what) {
        ...
        case NATIVE_WINDOW_HARDWARE_RENDER:
            value = (mCore->mConsumerName == "SurfaceView" && !mVirtualDevice);
            break;
        case NATIVE_WINDOW_SET_VIRTUAL_DEVICE:
            mVirtualDevice = *outValue;
            break;
       case NATIVE_WINDOW_HARDWARE_RENDER_DELAY_TIME:
            value = 0;
            MS_Video_Disp_Get_Cmd((MS_Win_ID)mOverlayID, MS_DISP_CMD_GET_RENDER_DELAY_TIME, &value, sizeof(value));
            break;
        case NATIVE_WINDOW_CONNECTED_API:
