...
status_t BufferQueueConsumer::acquireBuffer(BufferItem* outBuffer,
    ...
    // MStar Android Patch Begin
    if (mCore->mConsumerName == "SurfaceView") {
        while (front != mCore->mQueue.end()) {
            const BufferSlot &slot = mSlots[front->mSlot];
    ...
    if (expectedPresent != 0) {
        ...
        // MStar Android Patch Begin
        if (front->mGraphicBuffer.get() != NULL && (front->mGraphicBuffer->getUsage() & GRALLOC_USAGE_PRIVATE_2)) {
            //Do Nothing
        } else
        // MStar Android Patch End
    ...
    *outBuffer = *front;
    ...
    if (mCore->stillTracking(front)) {
        ...
        if (mSlots[slot].mGraphicBuffer != NULL) {
            if (mSlots[slot].mGraphicBuffer->getUsage() & GRALLOC_USAGE_PRIVATE_2) {
                // freeze
                ...
                if (u8FreezeMode) {
                    ...
                    if (MS_Video_Frame_Capture(MS_Win_ID(OverlayID), (MS_FreezeMode)u8FreezeMode,
                        &width, &height, &ms_win, out_buf, 0) >= 0) {

                        left = ms_win.x;
                        top = ms_win.y;
                        right = ms_win.x + ms_win.width;
                        bottom = ms_win.y + ms_win.height;
                        BQ_LOGD("Capture Video w = %d, h = %d, crop[%d %d %d %d]", width, height, left, top, right, bottom);

                        mSlots[slot].mYUVBuffer = 0;
                        if (u8FreezeMode == MS_FREEZE_MODE_VIDEO) {
                            mSlots[slot].mYUVBuffer = new GraphicBuffer(width, height, HAL_PIXEL_FORMAT_BGRA_8888, GRALLOC_USAGE_HW_TEXTURE | GRALLOC_USAGE_HW_RENDER | GRALLOC_USAGE_SW_WRITE_OFTEN);
                        } else {    // this is MS_FREEZE_MODE_BLANK_VIDEO
                            mSlots[slot].mYUVBuffer = new GraphicBuffer(width, height, HAL_PIXEL_FORMAT_RGBX_8888, GRALLOC_USAGE_HW_TEXTURE | GRALLOC_USAGE_HW_RENDER | GRALLOC_USAGE_SW_WRITE_OFTEN);
                        }
                        err = mSlots[slot].mYUVBuffer->lock(GRALLOC_USAGE_SW_WRITE_OFTEN, (void**)(&out_buf));
                        if (err == 0) {
                            int status = MS_Video_Frame_Capture(MS_Win_ID(OverlayID), (MS_FreezeMode)u8FreezeMode,
                                &width, &height, &ms_win, out_buf, mSlots[slot].mYUVBuffer->getStride());
                            mSlots[slot].mYUVBuffer->unlock();

                            if (status < 0) {
                                BQ_LOGE("vsync_bridge_capture_video failed\n");
                                mSlots[slot].mYUVBuffer = 0;
                            } else {
                                outBuffer->mGraphicBuffer = mSlots[slot].mYUVBuffer;
                            ...
                            BQ_LOGE("mSlots[buf].mYUVBuffer->lock(HW) failed: %d\n", err);
                            mSlots[slot].mYUVBuffer = 0;
                        }
                    }
                }
        ...
        if (bReplaceGraphicBuffer) {
            mSlots[slot].mAcquireCalled = false;        // clear to set buffer->mGraphicBuffer next time
            outBuffer->mAcquireCalled = false;          // remapping mGraphicBuffer to mYUVBuffer
        }
#endif
        // MStar Android Patch End
...
status_t BufferQueueConsumer::releaseBuffer(int slot, uint64_t frameNumber,
    ...
    { // Autolock scope
        ...
        if ((mConsumerName == "SurfaceView") && (mSlots[slot].mGraphicBuffer != NULL) && (mSlots[slot].mGraphicBuffer->getUsage() & GRALLOC_USAGE_PRIVATE_2)) {
        ...
        // MStar Android Patch Begin
        #ifdef RENDER_IN_QUEUE_BUFFER
        } else if ((mConsumerName == "SurfaceView") && (mSlots[slot].mBufferState == BufferSlot::FREE || mSlots[slot].mBufferState == BufferSlot::DEQUEUED ||
