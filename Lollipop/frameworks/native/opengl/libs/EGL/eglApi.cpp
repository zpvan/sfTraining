...
// MStar Android Patch Begin
EGLBoolean eglSwapBuffersWithDamageEXT(EGLDisplay dpy, EGLSurface draw, const EGLint *rects, EGLint n_rects)
{
    ATRACE_CALL();
    clearError();
    ...
    if (getEGLDebugLevel() > 0) {
        ...
        GLTrace_eglSwapBuffersWithDamageEXT(dpy, draw, rects, n_rects);
    } else if (trace_hooks != NULL) {
        // tracing is now disabled, so switch back to the non trace version
        EGLContext ctx = egl_tls_t::getContext();
        egl_context_t * const c = get_context(ctx);
    ...
    if (s->cnx->egl.eglSwapBuffersWithDamageEXT) {
        return s->cnx->egl.eglSwapBuffersWithDamageEXT(dp->disp.dpy, s->surface, rects, n_rects);
    }

    ALOGD("eglSwapBuffersWithDamageEXT is called with no implementation");
