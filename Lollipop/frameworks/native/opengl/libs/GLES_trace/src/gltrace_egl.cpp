...
void GLTrace_eglSwapBuffersWithDamageEXT(void* /*dpy*/, void* /*draw*/, const int *rects, int n_rects) {
    GLMessage glmessage;
    GLTraceContext *glContext = getGLTraceContext();

    glmessage.set_context_id(glContext->getId());
    glmessage.set_function(GLMessage::eglSwapBuffersWithDamageEXT);
    ...
    // copy argument rects
    GLMessage_DataType *arg_rects = glmessage.add_args();
    arg_rects->set_isarray(true);
    arg_rects->set_type(GLMessage::DataType::INT);
    arg_rects->clear_intvalue();

    for (int i = 0; i < n_rects; i++) {
        arg_rects->add_intvalue(rects[0]);
        arg_rects->add_intvalue(rects[1]);
        arg_rects->add_intvalue(rects[2]);
        arg_rects->add_intvalue(rects[3]);
        rects += 4;
    }
