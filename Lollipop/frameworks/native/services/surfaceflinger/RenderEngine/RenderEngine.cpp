...
uint32_t RenderEngine::initCaptureData(const intptr_t captureData, int opTimingWidth, int opTimingHeight) {
    uint32_t cameraCaptureTexture = 0;
    if (captureData != 0) {
        ssize_t offset;
        size_t size;
        ...
        glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, opTimingWidth, opTimingHeight, 0, GL_RGBA,
                 GL_UNSIGNED_BYTE,(uint8_t *)captureData);
