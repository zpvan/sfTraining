...
public:
    ...
    int isStbTarget();
    int isOsdNeedResize();
    int isFrameBufferResizeOpen();
    int bFrameBufferResizeOpen;
    // MStar Android Patch End
    ...
    // MStar Android Patch Begin
    nsecs_t getRefreshPeriod(int disp);
    // MStar Android Patch End
...
struct GameCursorData{
    ...
    GameCursorData mGameCursorData;
    int mCurrentTimingWidth;
    int mCurrentTimingHeight;
    // MStar Android Patch End
    ...
    // MStar Android Patch Begin
public:
    int setGameCursorShow();
    int setGameCursorHide();
    int setGameCursorMatrix(float dsdx, float dtdx, float dsdy, float dtdy);
    int setGameCursorPosition(float positionX, float positionY);
    int setGameCursorAlpha(float alpha);
    int redrawGameCursorFb();
    int doGameCursorTransaction();
    int setGameCursorWidth(int width);
    int setGameCursorHeight(int height);
    int setGameCursorStride(int stride);
    intptr_t getGameCursorFbVaddr();
    int initGameCursor();
    bool timingChanged();
    int redrawHwCursorFb(int srcX, int srcY, int srcWidth, int srcHeight, int stride, int dstWidth, int dstHeight);
    status_t updateFramebufferTargetFrameSize(int32_t id);
    status_t updateFramebufferTargetFrameSize(int32_t id, Rect &rect);
    status_t updateFramebufferTargetSourceCropSize(int32_t id, Rect &rect);
    void recalc3DTransform(Transform** first, Transform** second, Rect& frame);
    void captureScreen(uintptr_t* addr, int* width, int* height, int withOsd);
    struct CaptureData {
        uintptr_t address;
        Rect pos;
    };
    void captureScreenWithClip(DefaultKeyedVector< uintptr_t, CaptureData>& captureData, uintptr_t* addr, int* width, int* height, int withOsd);
    void releaseCaptureScreenMemory();
    int getMirrorMode();
    int isAFBCFramebuffer();
    // MStar Android Patch End
};
