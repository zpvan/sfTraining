...
status_t EventThread::Connection::postEvent(
    ...
    static int dropFlagClearCount;
    ssize_t size = DisplayEventReceiver::sendEvents(mChannel, &event, 1);
    // MStar Android Patch Begin
    if(size<0) {
        ALOGD("EventThread:postEvent fail, ret = %d, mCallingPid =%d", status_t(size), mCallingPid);
        mEventDroped = true;
        dropFlagClearCount = 10;
    } else if (mEventDroped) {
        dropFlagClearCount--;
        if(dropFlagClearCount == 0) {
            mEventDroped = false;
        }
    }
    // MStar Android Patch End
