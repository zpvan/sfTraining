...
#ifdef ENABLE_EGL_SWAP_BUFFERS_WITH_DAMAGE
...
public:
    ...
    PFNEGLSWAPBUFFERSWITHDAMAGEEXTPROC mEglSwapBuffersWithDamageEXT;
#endif
    // MStar Android Patch End
private:
    friend class Client;
    ...
    // MStar Android Patch Begin
    bool IsHwCursorLayer(const sp<Layer>& layer);
    // MStar Android Patch End
    ...
    void renderScreenImplLocked(
            ...
            uint32_t supportWidth = 1920,uint32_t supportHeight = 1080);
    // MStar Android Patch End
...
private:
    ...
    mutable Mutex mHwcursorLock;
    bool mOverScanChanged;
    bool mChangeTo2k1kMode;
    bool m4k2kAnd2k1kCoexistEnable;
    bool checkOsdResizeForResizeDeviceBuffer(const sp<DisplayDevice>& hw);
    bool check4k2kCoexistForResizeDeviceBuffer(const sp<DisplayDevice>& hw);
    bool checkStbResolutionForResizeDeviceBuffer(const sp<DisplayDevice>& hw);
public:
    ...
    void drawCaptureScreenData(const sp<const DisplayDevice>& hw, Texture texture, uint32_t supportWidth, uint32_t supportHeight);
    DefaultKeyedVector< uintptr_t, HWComposer::CaptureData> mCaptureData;
    void changeTransformAndRebuildLayerStacks();
    // MStar Android Patch End
};
