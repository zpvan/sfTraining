...
public class CompatibilityInfo implements Parcelable {
    ...
    /**
    *This is the number of pixels we would like to have along the
    * short axis of an app that needs to run on a 2k1k size screen.
    */
    public static final int DEFAULT_COMPATIBLE_MODE_2_DIMENSION = 720;
    // MStar Android Patch End
    ...
    // MStar Android Patch Begin
    /**
     * Set if the application needs to run in screen 2k1k size compatibility mode.
     */
    private static final int NEEDS_4K2K_COMPAT = 16;
    // MStar Android Patch Begin
    ...
    // MStar Android Patch Begin
    public static int mDefaultDimen = DEFAULT_COMPATIBLE_MODE_2_DIMENSION;
    public CompatibilityInfo(ApplicationInfo appInfo, int screenLayout, int sw,
            boolean forceCompat) {
            this(appInfo, screenLayout, sw, forceCompat, false);
    }
    public CompatibilityInfo(ApplicationInfo appInfo, int screenLayout, int sw,
            boolean forceCompat, boolean forceTo4K2KSize) {
    // MStar Android Patch End
        int compatFlags = 0;
        ...
        if (appInfo.requiresSmallestWidthDp != 0 || appInfo.compatibleWidthLimitDp != 0
            ...
            // MStar Android Patch Begin
            if (forceCompat || forceTo4K2KSize) {
            // MStar Android Patch End
    ...
    public boolean supportsScreen() {
        // MStar Android Patch Begin
        return !forceTo4k2kSize() && ((mCompatibilityFlags&NEEDS_SCREEN_COMPAT) == 0) ;
        // MStar Android Patch End
    }
    ...
    public void applyToDisplayMetrics(DisplayMetrics inoutDm) {
        if (!supportsScreen()) {
            ...
            // MStar Android Patch Begin
            float scale = CompatibilityInfo.computeCompatibleScaling(inoutDm, inoutDm);
            if (forceTo4k2kSize()) {
                float invertedRatio = 1 / scale;
                inoutDm.density = inoutDm.noncompatDensity * invertedRatio;
                inoutDm.densityDpi = (int)((inoutDm.noncompatDensityDpi * invertedRatio) + .5f);
                inoutDm.scaledDensity = inoutDm.noncompatScaledDensity * invertedRatio;
                inoutDm.xdpi = inoutDm.noncompatXdpi * invertedRatio;
                inoutDm.ydpi = inoutDm.noncompatYdpi * invertedRatio;
                inoutDm.widthPixels = (int) (inoutDm.noncompatWidthPixels * invertedRatio + 0.5f);
                inoutDm.heightPixels = (int) (inoutDm.noncompatHeightPixels * invertedRatio + 0.5f);
            }
            // MStar Android Patch End
        } else {
    ...
    public void applyToConfiguration(int displayDensity, Configuration inoutConfig) {
        if (!supportsScreen()) {
            ...
            // MStar Android Patch Begin
            if (forceTo4k2kSize()) {
                inoutConfig.screenWidthDp = inoutConfig.compatScreenWidthDp / 2;
                inoutConfig.screenHeightDp = inoutConfig.compatScreenHeightDp / 2;
                inoutConfig.smallestScreenWidthDp = inoutConfig.compatSmallestScreenWidthDp /2;
                inoutConfig.densityDpi = displayDensity * 2;
            } else {
    ...
    public static float computeCompatibleScaling(DisplayMetrics dm, DisplayMetrics outDm) {
        ...
        // MStar Android Patch Begin
        int newShortSize = 0;
        if (mDefaultDimen == DEFAULT_NORMAL_SHORT_DIMENSION) {
            newShortSize = (int)(DEFAULT_NORMAL_SHORT_DIMENSION * dm.density + 0.5f);
        } else {
            newShortSize = shortSize * 2;//(int)(DEFAULT_COMPATIBLE_MODE_2_DIMENSION * dm.density * 2.0f + 0.5f);
        }
        // MStar Android Patch End
        ...
        // MStar Android Patch Begin
        if (mDefaultDimen == DEFAULT_NORMAL_SHORT_DIMENSION && scale < 1) {
            scale = 1;
        }
        // MStar Android Patch End
        if (outDm != null) {
    ...
    // MStar Android Patch Begin
    public boolean forceTo4k2kSize() {
        return (mCompatibilityFlags&NEEDS_4K2K_COMPAT) != 0;
    }

    public static void setDefaultDinmen(boolean isForceTo4k2k) {
        mDefaultDimen = isForceTo4k2k? DEFAULT_COMPATIBLE_MODE_2_DIMENSION : DEFAULT_NORMAL_SHORT_DIMENSION;
    }

    public void applyToDisplayInfo(DisplayInfo inoutDi) {
        if (!supportsScreen()) {
            if (forceTo4k2kSize()) {
                DisplayMetrics tempMetrics = new DisplayMetrics();
                inoutDi.getAppMetrics(tempMetrics);
                float scale = CompatibilityInfo.computeCompatibleScaling(tempMetrics, tempMetrics);
                float invertedRatio = 1 / scale;
                inoutDi.appWidth = (int)(inoutDi.appWidth * invertedRatio + .5f);
                inoutDi.appHeight = (int)(inoutDi.appHeight * invertedRatio + .5f);
                inoutDi.logicalWidth = (int)(inoutDi.logicalWidth * invertedRatio + .5f);
                inoutDi.logicalHeight = (int)(inoutDi.logicalHeight * invertedRatio + .5f);
                inoutDi.logicalDensityDpi = (int)(inoutDi.logicalDensityDpi  * invertedRatio + .5f);
                inoutDi.physicalXDpi = inoutDi.physicalXDpi * invertedRatio;
                inoutDi.physicalYDpi = inoutDi.physicalYDpi * invertedRatio;
                inoutDi.smallestNominalAppWidth = (int)(inoutDi.smallestNominalAppWidth * invertedRatio + .5f);
                inoutDi.smallestNominalAppHeight = (int)(inoutDi.smallestNominalAppHeight * invertedRatio + .5f);
                inoutDi.largestNominalAppWidth = (int)(inoutDi.largestNominalAppWidth * invertedRatio + .5f);
                inoutDi.largestNominalAppHeight = (int)(inoutDi.largestNominalAppHeight * invertedRatio + .5f);
            }
        }
    }
    // MStar Android Patch End
