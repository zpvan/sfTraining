...
public class AudioService extends IAudioService.Stub {
    ...
    //Mstar patch for show volume ui begin
    private boolean isShowVolumeUI = false;

    public void setShowVolumeUI(boolean isShow){
        isShowVolumeUI = isShow;
    }
    //Mstar patch for show volume ui end
   ...
   /** @hide Maximum volume index values for audio streams */
    ...
    private static int[] DEFAULT_STREAM_VOLUME = new int[] {
        ...
        5,  // STREAM_RING
        // MStar Android Patch Begin
        // this setting is subject to MAX_STREAM_VOLUME[STREAM_MUSIC] = 100
        35, // STREAM_MUSIC
        // MStar Android Patch End
        6,  // STREAM_ALARM
    ...
    public void setStreamMute(int streamType, boolean state, IBinder cb) {
        ...
        if (isStreamAffectedByMute(streamAlias)) {
            ...
            if(isShowVolumeUI){
                sendVolumeUpdate(streamType, 0, 0, AudioManager.FLAG_SHOW_UI);
                isShowVolumeUI = false;
            }
            // MStar Android Patch End
        }
    }
    ...
    private void restoreMasterVolume() {
        ...
        if (mUseMasterVolume) {
            ...
            float volume = Settings.System.getFloatForUser(mContentResolver,
                    Settings.System.VOLUME_MASTER, 0.35f, UserHandle.USER_CURRENT);
            AudioSystem.setMasterVolume(volume);
            // MStar Android Patch end
        }
    }
    ...
    */
    ...
    public void setWiredDeviceConnectionState(int device, int state, String name) {
        ...
        // MStar Android Patch Begin
        // Set the speaker mute when HDMI ARC connected.
        if (device == AudioSystem.DEVICE_OUT_HDMI_ARC) {
            if (state == 1) {
                AudioSystem.setParameters("mute_speaker_by_user=true");
            } else {
                if (!mBluetoothA2dpEnabled) {
                   AudioSystem.setParameters("mute_speaker_by_user=false");
                }
            }
        }
        // MStar Android Patch End
    ...
    public int setBluetoothA2dpDeviceConnectionState(BluetoothDevice device, int state, int profile)
    {
        ...
        // MStar Android Patch Begin
        // Set the speaker mute when bluetooth connected.
        if (profile == BluetoothProfile.A2DP) {
            if (state == BluetoothA2dp.STATE_CONNECTED) {
                AudioSystem.setParameters("mute_speaker_by_user=true");
            } else if (state == BluetoothA2dp.STATE_DISCONNECTED) {
                // To do : Set the speaker unmute when the HDMI ARC isn't connected,
                // or don't do anything when the HDMI ARC connected.
                AudioSystem.setParameters("mute_speaker_by_user=false");
            }
        }
        // MStar Android Patch End
    ...
    public class VolumeStreamState {
        ...
        public void readSettings() {
            synchronized (VolumeStreamState.class) {
                ...
                for (int i = 0; remainingDevices != 0; i++) {
                    ...
                    // MStar Android Patch Begin
                    // FIXME: The database of system volume haven't index of speaker and a2dp before adjust volume
                    // If haven't, we need use default volume insert the map of volume index
                    if (((device == AudioSystem.DEVICE_OUT_SPEAKER) || (device == AudioSystem.DEVICE_OUT_BLUETOOTH_A2DP))
                        && (mStreamType == AudioSystem.STREAM_MUSIC)
                        && (index == -1)) {
                        index = DEFAULT_STREAM_VOLUME[mStreamType];
                    }
                    // MStar Android Patch End
        ...
        public void applyAllVolumes() {
            synchronized (VolumeStreamState.class) {
                ...
                int index;
                // MStar Android Patch Begin
                // FIXME: Don't apply default volume before apply device volume
                // If the default volume is very different from the speaker volume, this process sounds like a pop noise
                //if (mIsMuted) {
                //    index = 0;
                //} else {
