...
public class AudioManager {
        ...
        switch (keyCode) {
            ...
            case KeyEvent.KEYCODE_VOLUME_MUTE:
                if (event.getRepeatCount() == 0) {
                   // MStar Android Patch Begin
                   // Force to use Android 4.4 flow in advance
                   //- MediaSessionLegacyHelper.getHelper(mContext).sendVolumeKeyEvent(event, false);
                    if (mUseMasterVolume) {
                        ...
                        try {
                            ...
                            service.setShowVolumeUI(true);
                        } catch (RemoteException e) {
                            Log.e(TAG, "Dead object handleKeyDown==case===KEYCODE_VOLUME_MUTE", e);
                        }
                        MediaSessionLegacyHelper.getHelper(mContext).sendVolumeKeyEvent(event, false);
                    }
                   // MStar Android Patch End
    ...
    public void adjustStreamVolume(int streamType, int direction, int flags) {
        // MStar Android Patch Begin
        if(getRingerMode() != RINGER_MODE_NORMAL){
            Log.w(TAG, "adjustStreamVolume kelvin.wei patch setRingerMode to RINGER_MODE_NORMAL");
    ...
    public void adjustVolume(int direction, int flags) {
        // MStar Android Patch Begin
        if(getRingerMode() != RINGER_MODE_NORMAL){
            Log.w(TAG, "adjustVolume kelvin.wei patch setRingerMode to RINGER_MODE_NORMAL");
            setRingerMode(RINGER_MODE_NORMAL);
        }
        // MStar Android Patch End
        IAudioService service = getService();
    ...
    public void adjustMasterVolume(int steps, int flags) {
        // MStar Android Patch Begin
        if(getRingerMode() != RINGER_MODE_NORMAL){
            Log.w(TAG, "adjustMasterVolume kelvin.wei patch setRingerMode to RINGER_MODE_NORMAL");
            setRingerMode(RINGER_MODE_NORMAL);
        }
        // MStar Android Patch End
        IAudioService service = getService();
    ...
    public void setMasterMute(boolean state) {
        // MStar Android Patch Begin
        if(getRingerMode() != RINGER_MODE_NORMAL){
            Log.w(TAG, "setMasterMute kelvin.wei patch setRingerMode to RINGER_MODE_NORMAL");
            setRingerMode(RINGER_MODE_NORMAL);
        }
        // MStar Android Patch End
    ...
    */
    ...
    // MStar Android Patch Begin
    /**
     * Request use of Bluetooth MIC Input for Audio
     * <p>
     * This method should only be used by applications that replace the platform-wide
     * management of audio settings or the main telephony application.
     *
     * @hide
     * @param on set <var>true</var> to use Bluetooth MIC Input for Audio;
     *               <var>false</var> to not use Bluetooth MIC Input for Audio
     @param bi_directional  set <var>true</var>to use both speaker and microphone path
     *               <var>false</var> to use only the Microphone path
     */

    public void setBluetoothMicOn(boolean on,boolean bi_directional){
        IAudioService service = getService();
        try {
            service.setBluetoothMicOn(on,bi_directional);
        } catch (RemoteException e) {
            Log.e(TAG, "Dead object in setBluetoothMicOn", e);
        }
    }

    /**
     * Checks whether communications use Bluetooth MIC.
     *
     * @hide
     * @return true if Bluetooth MIC is used for communications;
     *         false if otherwise
     */
    public boolean isBluetoothMicOn() {
        IAudioService service = getService();
        try {
            return service.isBluetoothMicOn();
        } catch (RemoteException e) {
            Log.e(TAG, "Dead object in isBluetoothMicOn", e);
            return false;
        }
    }
    // MStar Android Patch End
    ...
    // MStar Android Patch Begin
    /** @hide
     * The audio output device code for line out.
     */
    public static final int DEVICE_OUT_LINE_OUT = AudioSystem.DEVICE_OUT_LINE_OUT;
    // MStar Android Patch End
