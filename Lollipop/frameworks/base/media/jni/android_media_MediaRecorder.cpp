...
android_media_MediaRecorder_setOutputFormat(JNIEnv *env, jobject thiz, jint of)
{
    ...
    if (of < OUTPUT_FORMAT_DEFAULT ||
        (of >= OUTPUT_FORMAT_LIST_END && of < OUTPUT_FORMAT_MSTAR_START) ||
        of >= OUTPUT_FORMAT_MSTAR_START) {
...
static void
android_media_MediaRecorder_native_pause(JNIEnv *env, jobject clazz)
{
    ALOGI("android_media_MediaRecorder_native_pause");
    
    sp<MediaRecorder> ms = getMediaRecorder(env, clazz);
    process_media_recorder_call(env, ms->pause(), "java/lang/RuntimeException", "resume failed.");

}

static void
android_media_MediaRecorder_native_resume(JNIEnv *env, jobject clazz)
{
    ALOGI("android_media_MediaRecorder_native_resume");
    
    sp<MediaRecorder> mr = getMediaRecorder(env, clazz);
...
static JNINativeMethod gMethods[] = {
    ...
    { "native_pause",        "()V",                             (void *)android_media_MediaRecorder_native_pause},
    { "native_resume",       "()V",                             (void *)android_media_MediaRecorder_native_resume},
};
