...
decVideoSurfaceRef(JNIEnv *env, jobject thiz)
{
    ...
    sp<IGraphicBufferProducer> tmp;
    tmp = getVideoSurfaceTexture(env, thiz);
    sp<IGraphicBufferProducer> old_st = tmp;
    // MStar Android Patch End
    if (old_st != NULL) {
...
android_media_MediaPlayer_getCurrentPosition(JNIEnv *env, jobject thiz)
{
    sp<MediaPlayer> mp = getMediaPlayer(env, thiz);
    if (mp == NULL ) {
        ALOGE("[Warning] Native MediaPlayer has already been released, get current position return 0");
        return 0;
    }
    int msec;
...
static jint
android_media_MediaPlayer_switchChannel(JNIEnv *env, jobject clazz, jint channel)
{
    ALOGI("android_media_MediaPlayer_switchChannel");

    sp<MediaPlayer> mp = getMediaPlayer(env, clazz);
    if (mp == NULL) {
        jniThrowException(env, "java/lang/IllegalStateException", NULL);
        return 0;
    }

    status_t err = mp->setAudioTrack(channel);
    if (err == OK) {
        return 1;
    } else {
        return 0;
...
static jint
android_media_MediaPlayer_canSelectTrackImmediately(JNIEnv *env, jobject clazz)
{
    ALOGI("android_media_MediaPlayer_canSelectTrackImmediately");
    return 1;
}
...
static JNINativeMethod gMethods[] = {
    ...
    { "native_switchChannel", "(I)I",                           (void *)android_media_MediaPlayer_switchChannel},
    { "native_canSelectTrackImmediately", "()I",                (void *)android_media_MediaPlayer_canSelectTrackImmediately},
};
