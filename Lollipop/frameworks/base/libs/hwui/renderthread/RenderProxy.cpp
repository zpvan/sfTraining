...
void* RenderProxy::postAndWait(MethodInvokeRenderTask* task) {
    ...
    nsecs_t  waitTime  ;
    nsecs_t  endWait   ;
    nsecs_t  beginWait = systemTime(SYSTEM_TIME_MONOTONIC);
    //checkout timeout interval
    //set 200 ms interval to check if render task is timeout
    nsecs_t  condCheckTime = 200000000LL ;
    //if wait render task not finish for 10 sec, watch dog will act as system hang
    //we can not wait render task too long
    //we set 6 sec as the maximal waiting time for render task
    nsecs_t  timeOut = 6000000000LL;

    while (!syncTask.done()) {
        retStatus = mSyncCondition.waitRelative(mSyncMutex,condCheckTime);
        if (retStatus != 0) {
            //exception handle
            if (retStatus == -ETIMEDOUT) {
                //need check if render task is hang
                endWait  = systemTime(SYSTEM_TIME_MONOTONIC);
                waitTime = endWait - beginWait ;
                if (waitTime >=  timeOut) {
                    ALOGE("render task handle too long and time out reached(%lld)ms",waitTime / 1000000LL);
                    break;
                }
            } else {
                //other exception
                ALOGE("exception happen(%d) when condition wait",retStatus);
                break;
            }
        }
    }
...
void* RenderProxy::staticPostAndWait(MethodInvokeRenderTask* task) {
    ...
    while (!syncTask.done()) {
        retStatus = condition.waitRelative(mutex,condCheckTime);
        if (retStatus != 0) {
            //exception handle
            if (retStatus == -ETIMEDOUT) {
                //need check if render task is hang
                ...
                waitTime = endWait - beginWait;
                if (waitTime >=  timeOut) {
                    ALOGE("static post and wait, render task handle too long and time out reached(%lld)ms",waitTime / 1000000LL);
                    break;
                }
            } else {
                //other exception
                ALOGE("static post and wait, exception happen(%d) when condition wait",retStatus);
                break;
            }
        }
    }
