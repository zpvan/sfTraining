...
public class AnimationDrawable extends DrawableContainer implements Runnable, Animatable {
    ...
    public boolean setVisible(boolean visible, boolean restart) {
        final String _tmp = android.util.Log.getStackTraceString(new Throwable());
        final boolean changed = super.setVisible(visible, restart);
        if (visible) {
            if (restart || changed) {
                boolean startFromZero = restart || mCurFrame < 0 ||
                        mCurFrame >= mAnimationState.getChildCount();
                setFrame(startFromZero ? 0 : mCurFrame, true, mAnimating||_tmp.contains("com.bestv.ott.jingxuan.view.VideoLoadingView.showAllElements"));
            }
        } else {
            unscheduleSelf(this);
        }
