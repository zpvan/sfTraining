...
import android.content.ComponentName;
import android.os.display.DisplayManager;  // For C2_CMCC
...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    static final boolean Is_C2_CMCC = "cmcc".equals(android.os.SystemProperties.get("ro.operator.project", "na"));
    ...
    // MStar Android Patch Begin, for C2_CMCC
    static final int LONG_PRESS_POWER_CHANGE_RESOLUTION = 4;
    // MStar Android Patch End, for C2_CMCC
        ...
        if (!mPowerKeyHandled) {
            if (interactive) {
                ...
                if (hasLongPressOnPowerBehavior()) {
                    // MStar Android Patch Begin
                    if (enable_str == false || Is_C2_CMCC) {
    ...
    private void interceptPowerKeyUp(KeyEvent event, boolean interactive, boolean canceled) {
        ...
        if (!handled) {
            ...
            // MStar Android Patch Begin, for C2_CMCC
            if (Is_C2_CMCC && mPowerKeyPressCounter == 1){
                Log.d(TAG,"Powerkey Press one time, shut down!!!!!!!!");
        ...
        if (count == 2) {
            ...
            switch (mShortPressOnPowerBehavior) {
                ...
                case SHORT_PRESS_POWER_REALLY_GO_TO_SLEEP:
                    // MStar Android Patch Begin, for C2_CMCC
                    if (Is_C2_CMCC && enable_str==false){
    ...
    // MStar Android Patch Begin, for C2_CMCC
    private void changeScreenResulotion() {
        if (SystemProperties.get("ms.mstplayer.mvc", "0").contains("1")){
            Toast.makeText(mContext, "3D节目无法切换显示模式，请退出后再设置", Toast.LENGTH_LONG).show();
            return;
        }
        Log.d(TAG,"changeScreenResulotion to 720p");
        DisplayManager dm = (DisplayManager)mContext.getSystemService(mContext.MDISPLAY_MANAGER_SERVICE);
        //set resulotion to 720p, 8 is DISPLAY_STANDARD_720P_50
        dm.setDisplayStandard(DisplayManager.DISPLAY_STANDARD_720P_50);
        Intent intent = new Intent("android.intent.action.screenSize");
        mContext.sendBroadcast(intent);
        Toast.makeText(mContext, "已切换到 720P 显示模式", Toast.LENGTH_LONG).show();
    }
    // MStar Android Patch End, for C2_CMCC
    ...
    private void powerLongPress() {
        ...
        case LONG_PRESS_POWER_SHUT_OFF_NO_CONFIRM:
            ...
            break;
        // MStar Patch Android Begin, for C2_CMCC
        case LONG_PRESS_POWER_CHANGE_RESOLUTION:
            mPowerKeyHandled = true;
            changeScreenResulotion();
            break;
        // MStar Patch Android End, for C2_CMCC
        }
    }
    ...
    boolean isUserSetupComplete() {
        Slog.w(TAG, "force return isUserSetupComplete==true");
        return true;
        // we don't need the SetupWizard to change USER_SETUP_COMPLETE
    ...
    public void init(Context context, IWindowManager windowManager,
        ...
        if (Is_C2_CMCC) {
            mLongPressOnPowerBehavior = mContext.getResources().getInteger(
                com.android.internal.R.integer.config_cmcc_longPressOnPowerBehavior);
        } else {
            mLongPressOnPowerBehavior = mContext.getResources().getInteger(
...
//MStar Android Patch Begin
    private String getCurRunningActivityPackageName() {
        ...
        try {
            ...
            Log.i(TAG,"have no permission to getCurRunningActivityPackageName");
        }
...
//MStar Android Patch End
    ...
    public long interceptKeyBeforeDispatching(WindowState win, KeyEvent event, int policyFlags) {
        ...
        Log.e(TAG,"keyCode = " + keyCode + " down = " + down);
        ...
        //MStar Android Patch Begin
        if(down && keyCode == KeyEvent.KEYCODE_SETTINGS) {
            // MStar Android Patch Begin, for C2_CMCC
            if (Is_C2_CMCC){
                if(repeatCount<10){
                    ...
                    //show the entrance of bluetooth & miracast
                    Intent intent = new Intent();
                    intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                    intent.setClassName("com.mstar.cmcchomemedia","com.mstar.cmcchomemedia.MainActivity");
                    mContext.startActivity(intent);
                    return -1;
                }
            ...
            // MStar Android Patch End, for C2_CMCC
            String val = "";
            val = SystemProperties.get("ro.hotkey.setting");
            if (val.equals(""))
            {
                val = "com.android.tv.settings";
                Log.e(TAG,"val = " + val);
            }
            Intent intent = mContext.getPackageManager().getLaunchIntentForPackage(val);
            if (intent != null) {
                intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                Log.e(TAG, "Now to start Activity " + val);
                mContext.startActivity(intent);
            }
            return -1;
        }
        ...
        if(keyCode == KeyEvent.KEYCODE_AVR_POWER){
            String cur_process = getCurRunningActivityPackageName();
            String val = "";
            val = SystemProperties.get("ro.hotkey.homepage");
            Log.e(TAG,"cur_process = " + cur_process);
            if (val.equals(cur_process)){
                Log.d(TAG,"Current Activity is IPTV");
                //return 0;
            } else{
                Intent intent = mContext.getPackageManager().getLaunchIntentForPackage(val);
                intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
        ...
        if (
            ...
            String current_process = getCurRunningActivityPackageName();
            if ("com.android.tv.settings".equals(current_process)) {
                Log.d(TAG,"Current Activity is Settings");
            } else {
                if (down) {
                    ComponentName componentname = new ComponentName("com.android.tv.settings", "com.android.tv.settings.system.KeyboardActivity");
                    Intent intent = new Intent();
                    intent.setComponent(componentname);
                    intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                    Log.d(TAG, "Now to start Activity keyboard");
                    mContext.startActivity(intent);
                    return -1;
                }
            }
        ...
        // MStar Android Patch Begin, for C2_CMCC
        // CMCC need to handle F12, to start up xiri
        if (Is_C2_CMCC && keyCode == KeyEvent.KEYCODE_F12 && down) {
            String Packagename = SystemProperties.get("epg.xiri.regist",null);
            String xunfeiAction = SystemProperties.get("epg.xunfei.action",null);
            if("".equals(Packagename) && !"".equals(xunfeiAction)) {
                Intent intent = new Intent(xunfeiAction);
                intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                mContext.startActivity(intent);
                return -1;
            }
        ...
        if (keyCode == KeyEvent.KEYCODE_HOME) {
            ...
            // MStar Android Patch Begin, for C2_CMCC
            // start CMCC launcher, reference 2.1.7.12_M3.pdf, sync from http://szgit01:8090/#/c/9015/
            else if (down && repeatCount == 0){
                if (Is_C2_CMCC){
                    String CMCCLauncherPackagename = SystemProperties.get("epg.launcher.packagename");
                    PackageInfo packageinfo = null;
                    if (CMCCLauncherPackagename == null || "".equals(CMCCLauncherPackagename)) {
                         Log.w(TAG, "epg.launcher.packagename is not set");
                    }
                    else if(SystemProperties.get("ro.product.name").contains("bestv")){
                            Intent intent_bestv =  mContext.getPackageManager().getLaunchIntentForPackage(CMCCLauncherPackagename);
                            if(intent_bestv != null) {
                            intent_bestv.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                            mContext.startActivity(intent_bestv);
                            }
                            return -1;
                    } else {
                        try {
                            PackageManager pm = mContext.getPackageManager();
                            packageinfo = pm.getPackageInfo(CMCCLauncherPackagename, 0);
                            Intent tmpintent = new Intent(Intent.ACTION_MAIN, null);
                            tmpintent.setPackage(packageinfo.packageName);
                            List<ResolveInfo> apps = pm.queryIntentActivities(tmpintent, 0);
            ...
            //for cucc homepage key
            String homepage_val = "";
            String current_process = getCurRunningActivityPackageName();
            homepage_val = SystemProperties.get("ro.hotkey.mainpage");
            if (!homepage_val.equals(""))
            {
                Intent intent = mContext.getPackageManager().getLaunchIntentForPackage(homepage_val);     
                if (intent != null) {
                    intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
                    Log.e(TAG, "Now to start Activity " + homepage_val);
                    mContext.startActivity(intent);
                }
                return -1;
            }
   ...
   // Process Key First for Wake up When StandBy Record
    ...
    public int interceptKeyBeforeQueueing(KeyEvent event, int policyFlags) {
        ...
        if (enable_str == true) {
            //mstar.str.suspending is 1(suspending or resuming) or 0(other states).
        ...
        switch (keyCode) {
            ...
            case KeyEvent.KEYCODE_VOLUME_MUTE: {
				DisplayManager mDisplayManager = (DisplayManager)mContext.getSystemService(mContext.MDISPLAY_MANAGER_SERVICE);
				Log.d(TAG, "mDisplayManager is " + mDisplayManager);
				if (keyCode == KeyEvent.KEYCODE_VOLUME_DOWN) {
					mDisplayManager.SetColorSpaceRange(0);
                    if (down) {
					...
					mDisplayManager.SetColorSpaceRange(1);
                    if (down) {
                ...
                if (down) {
                    ...
                    // MStar Android Patch Begin
                    if (Is_C2_CMCC & (keyCode == KeyEvent.KEYCODE_VOLUME_DOWN
                        || keyCode == KeyEvent.KEYCODE_VOLUME_UP)) {
                        // volume up/down unmute
                        android.media.AudioManager mAudioManager = new android.media.AudioManager(mContext);
                        if (mAudioManager.isMasterMute()
                            || mAudioManager.isStreamMute(android.media.AudioManager.USE_DEFAULT_STREAM_TYPE)){
                            Log.w(TAG, "volume up/down unMute");
                            mAudioManager.setMasterMute(false);
                            mAudioManager.setStreamMute(android.media.AudioManager.USE_DEFAULT_STREAM_TYPE, false);
                        }
                    }
                    // MStar Android Patch End
                }
