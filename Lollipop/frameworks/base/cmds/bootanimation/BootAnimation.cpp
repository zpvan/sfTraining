...
//MStar Android Patch End
    ...
    if ( ((property_get("mstar.bootvideo", decrypt, NULL) > 0) && (access(decrypt, F_OK) == 0) ) ||
        (access(USER_BOOTVIDEO_FILE, F_OK) == 0)) {
        mVideo = true;
    if(property_get("bootvideo.bootanimation", decrypt, 0) != 1){
        mZip = NULL;
      }
        ...
        if(property_get("mstar.close.bootlogo.gop.frame", decrypt, NULL) <= 0 ) {
            property_set("mstar.close.bootlogo.gop.frame", "2");
        }
    }
    // MStar Android Patch End
    return NO_ERROR;
...
bool BootAnimation::threadLoop()
{
    ...
    char video_finish_property[PROPERTY_VALUE_MAX] = {0};
    char bootvideo_bootanimation[PROPERTY_VALUE_MAX] = {0};
    property_get("mstar.hw.init", property, "0");
    property_get("mstar.videoadvert.finished", video_finish_property, "0");
    property_get("bootvideo.bootanimation", bootvideo_bootanimation, "0");
    int hwInit = atoi(property);
    int videoFinish = atoi(video_finish_property);
    int bootvideoBootanimation = atoi(bootvideo_bootanimation);
    if (hwInit == 0) {
        int exitnow;
    ...
    if(bootvideoBootanimation == 1){
       if (mVideo && videoFinish != 1) {
        r = video();
    }
       if (mZip != NULL ) {
        r = movie();
       ...
       if (mZip == NULL ) {
        // Grandbeing Android Patch Begin
        // r = android();
        // Grandbeing Android Patch End
    }
       }
    else{
       if (mVideo) {
