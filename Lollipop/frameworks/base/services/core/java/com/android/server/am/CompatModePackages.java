...
public final class CompatModePackages {
    ...
    // MStar Android Patch Begin
    static final String UHD_KEEPING_PKGS_PATH = "/system/etc/uhd_keeping_packages.xml";
    final ArrayList<String> mUhdKeepingPkgs = new ArrayList<String>();
    // MStar Android Patch End
    ...
    public CompatModePackages(ActivityManagerService service, File systemDir, Handler handler) {
        ...
        // MStar Android Patch Begin
        //parsePackagesList("utf-8", UHD_KEEPING_PKGS_PATH, mUhdKeepingPkgs);
        // MStar Android Patch End
    }
    ...
    public CompatibilityInfo compatibilityInfoForPackageLocked(ApplicationInfo ai) {
        // MStar Android Patch Begin
        boolean enable = SystemProperties.getBoolean("mstar.4k2k.2k1k.coexist", false);
        CompatibilityInfo ci = null;
        if (enable) {
            ci = new CompatibilityInfo(ai, mService.mConfiguration.screenLayout,
                    mService.mConfiguration.smallestScreenWidthDp,
                    (getPackageFlags(ai.packageName)&COMPAT_FLAG_ENABLED) != 0,
                    mUhdKeepingPkgs.contains(ai.packageName));
        } else {
            ci = new CompatibilityInfo(ai, mService.mConfiguration.screenLayout,
                mService.mConfiguration.smallestScreenWidthDp,
                (getPackageFlags(ai.packageName)&COMPAT_FLAG_ENABLED) != 0);
            ...
            while (parser.getEventType() != XmlPullParser.END_DOCUMENT) {
                if (parser.getEventType() == XmlPullParser.START_TAG) {
                    if (parser.getName().equalsIgnoreCase("item")) {  // get tag name
                        ...
                        pkgList.add(pkg);
                        Slog.i(TAG, "parsePackagesList>> adding pkg=" + pkg);
                    }
                }
                parser.next();
            }
