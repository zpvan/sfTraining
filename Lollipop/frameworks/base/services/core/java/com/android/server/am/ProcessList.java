...
final class ProcessList {
    ...
    private void updateOomLevels(int displayWidth, int displayHeight, boolean write) {
        ...
        String property_minfree = SystemProperties.get("ro.mstar.lmkd.minfree", "");
        String property_adj = SystemProperties.get("ro.mstar.lmkd.adj", "");
        if (!"".equals(property_minfree) && !"".equals(property_adj)) {
            String[] oomMinFree = property_minfree.split(",", mOomAdj.length);
            String[] oomAdj = property_adj.split(",", mOomAdj.length);
            for (int i=0; i<mOomAdj.length; i++) {
                mOomMinFree[i] = Integer.valueOf(oomMinFree[i]);
                mOomAdj[i] = Integer.valueOf(oomAdj[i]);
            }
        }
        ...
        if (write) {
            ...
            for (int i=0; i<mOomAdj.length; i++) {
                ...
                // MStar Android Patch Begin
                // Rearrange value for kernel
                //buf.putInt(mOomAdj[i]);
                buf.putInt((mOomAdj[i] * 1000) / 17);
                // MStar Android Patch End
            }
