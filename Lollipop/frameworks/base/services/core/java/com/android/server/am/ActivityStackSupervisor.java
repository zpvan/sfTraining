...
import static com.mstar.android.MIntent.FLAG_ACTIVITY_INHERIT_ATTRIBUTE;
...
public final class ActivityStackSupervisor implements DisplayListener {
    ...
    // MStar Android Patch Begin
    static boolean mCpuDvfsScaling = false;
    static boolean mBenchmarkAcceleratorEnabled = false;
    static String mCpuDvfsScalingCurrentTask = "";
    final ArrayList<String> mCpuDvfsScalingPkgs = new ArrayList<String>();
    final ArrayList<String> mBenchmarkAcceleratorPkgs = new ArrayList<String>();
    static final String CPU_DVFS_SCALING_PKGS_PATH = "/system/etc/cpu_dvfs_scaling_packages.xml";
    static boolean mVideoTalk = false;
    final ArrayList<String> mVideoTalkPkgs = new ArrayList<String>();
    static String mVideoTalkCurrentTask = "";
    static final String VIDEO_TALK_PKGS_PATH = "/system/etc/videotalk_packages.xml";
    static boolean mDisableOverscan = false;
    final ArrayList<String> mDisableOverscanPkgs = new ArrayList<String>();
    static String mOverscanCurrentTask = "";
    static final String DISPLAY_OVERSCAN_PKGS_PATH = "/system/etc/disable_overscan_packages.xml";
    static boolean mUseSwEncoder = false;
    final ArrayList<String> mUseSwEncoderPkgs = new ArrayList<String>();
    static String mSwEncoderCurrentTask = "";
    static final String CAMERA_SWENC_PKGS_PATH = "/system/etc/camera_software_enc_packages.xml";
    static boolean mEnableMultiPlayer = false;
    final ArrayList<String> mEnableMultiPlayerPkgs = new ArrayList<String>();
    static String mEnableMultiPlayerCurrentTask = "";
    static final String MULTI_PLAYER_PKGS_PATH = "/system/etc/multi_player_packages.xml";
    // MStar Android Patch End
    ...
    public ActivityStackSupervisor(ActivityManagerService service) {
        ...
        // MStar Android Patch Begin
        mCpuDvfsScaling = initCpuDvfsScaling();
        mBenchmarkAcceleratorEnabled = initBenchmarkAccelerate();
        mVideoTalk = initVideoTalk();
        mDisableOverscan = initDisableOverscanpkgs();
        mUseSwEncoder = initUseSwEncoderPkgs();
        mEnableMultiPlayer = initEnableMultiPlayerpkgs();
        // MStar Android Patch End
    }
    ...
    final boolean realStartActivityLocked(ActivityRecord r,
        ...
        try {
            ...
            // MStar Android Patch Begin
            if (!r.compat.supportsScreen()) {
                CompatibilityInfo.setDefaultDinmen(r.compat.forceTo4k2kSize());
                Configuration config = mWindowManager.updateOrientationFromAppTokens(
                    mService.mConfiguration,
                    r.mayFreezeScreenLocked(app) ? r.appToken : null);
                mService.updateConfigurationLocked(config, r, false, false);
    ...
    final int startActivityLocked(IApplicationThread caller,
        ...
        if (err == ActivityManager.START_SUCCESS) {
			...
			boolean Is_C2_CMCC = "cmcc".equals(android.os.SystemProperties.get("ro.operator.project", "na"));
			if (Is_C2_CMCC){
	            String tmp = ""+intent.toShortString(true, true, true, false);
	            android.content.ContentResolver cr = mService.mContext.getContentResolver();
	            int wire_mode = android.provider.Settings.Secure.getInt(cr, "default_eth_mod", 2);

	            if( tmp.contains("net.sunniwell.app.guide.chinamobile/.LanDhcp") ){
	               // if(  wire_mode != 0 ){  // wire mode is not dhcp, || ethernet is not connected
	                android.provider.Settings.Secure.putInt(cr,"default_eth_mod",0);
	                {
	                    android.net.ethernet.EthernetManager.getInstance().setEthernetMode("dhcp", new android.net.DhcpInfo() );
	                    android.net.ethernet.EthernetManager.getInstance().setEnabled(false);
	                    android.net.ethernet.EthernetManager.getInstance().setEnabled(true);
	                }
	                Intent __intent = new Intent("android.net.ethernet.ETHERNET_STATE_CHANGE");
	                __intent.addFlags(Intent.FLAG_RECEIVER_REGISTERED_ONLY_BEFORE_BOOT);
	                __intent.putExtra(android.net.ethernet.EthernetManager.EXTRA_ETHERNET_STATE,
	                            android.net.ethernet.EthernetManager.EVENT_DHCP_CONNECT_SUCCESSED);
	                mService.mContext.sendStickyBroadcastAsUser(__intent, UserHandle.ALL);
	            }
	        }
		}
        ...
        if (((launchFlags & Intent.FLAG_ACTIVITY_NEW_TASK) != 0 &&
            ...
            if (inTask == null && r.resultTo == null) {
                ...
                if (intentActivity != null) {
                    ...
                    // MStar Android Patch Begin
                    int prevTopOfHome = intentActivity.task.getTaskToReturnTo();
                    // MStar Android Patch End
                    ...
                    if (curTop != null && (curTop.task != intentActivity.task ||
                        ...
                        if (sourceRecord == null || (sourceStack.topActivity() != null &&
                            ...
                            // MStar Android Patch Begin
                            if ((launchFlags &
                                    (FLAG_ACTIVITY_NEW_TASK | FLAG_ACTIVITY_INHERIT_ATTRIBUTE))
                                    == (FLAG_ACTIVITY_NEW_TASK | FLAG_ACTIVITY_INHERIT_ATTRIBUTE)) {
                                intentActivity.task.setTaskToReturnTo(prevTopOfHome);
                            }
                            // MStar Android Patch End
                            options = null;
                            movedToFront = true;
    ...
    final ActivityRecord activityIdleInternalLocked(final IBinder token, boolean fromTimeout,
        ...
        if ("0".equals(SystemProperties.get("mstar.videoadvert.finished", "1"))) {
            Slog.i(TAG, "Boot video doesn't finish yet, wait it!!!");
            return null;
        }
        // MStar Android Patch End
    ...
    void checkReadyForSleepLocked() {
        ...
        if (mGoingToSleep.isHeld()) {
            ...
            // MStar Android Patch Begin
            if (enable_str) {
                SystemProperties.set("mstar.str.activityready", "true");
            }
            // MStar Android Patch End
        }
...
*/
    ...
    // MStar Android Patch Begin
    void dumpRecentTasks() {
        Slog.v(TAG, "dumpRecentTasks >> getRecentTasksSize()=" + mService.getRecentTasksSize());
        Slog.v(TAG, "getMostRecentTask=" + mService.getMostRecentTask());
        Slog.v(TAG, "getMost2ndRecentTask=" + mService.getMost2ndRecentTask());
    }
    ...
    private boolean initDisableOverscanpkgs() {
        return parsePackagesList("utf-8", DISPLAY_OVERSCAN_PKGS_PATH, mDisableOverscanPkgs);
    }
    void doDisableOverscan() {
        if (!mDisableOverscan) {
            return;
        }
        TaskRecord axr = mService.getMostRecentTask();
        if (axr == null) {
            // should not happen
            return;
        }
        String pkgName = axr.intent.getComponent().getPackageName();

        // to ensure the current task has changed
        if (mOverscanCurrentTask.equals(pkgName)) {
            return;
        }
        mOverscanCurrentTask = pkgName;

        if (mDisableOverscanPkgs.contains(pkgName)) {
            SystemProperties.set("mstar.disable.overscan","1");
            Slog.i("jermy", "disable.overscan = " + pkgName);
        } else {
            SystemProperties.set("mstar.disable.overscan","0");
            Slog.i("jermy", "enable.overscan = " + pkgName);
        }
    }
    private boolean initEnableMultiPlayerpkgs() {
        return parsePackagesList("utf-8", MULTI_PLAYER_PKGS_PATH, mEnableMultiPlayerPkgs);
    }
    void doEnableMultiPlayerSetting() {
        if (!mEnableMultiPlayer) {
            return;
        }
        TaskRecord axr = mService.getMostRecentTask();
        if (axr == null) {
        ...
        // to ensure the current task has changed
        if (mEnableMultiPlayerCurrentTask.equals(pkgName)) {
            return;
        }
        mEnableMultiPlayerCurrentTask = pkgName;

        if (mEnableMultiPlayerPkgs.contains(pkgName)) {
            SystemProperties.set("mstar.media.enable_multi_player","1");
            Slog.i("multiplayer", "enable multiplayer" + pkgName);
        } else {
            SystemProperties.set("mstar.media.enable_multi_player","0");
            Slog.i("multiplayer", "disable multiplayer" + pkgName);
        }
    }

    private boolean initUseSwEncoderPkgs() {
        return parsePackagesList("utf-8", CAMERA_SWENC_PKGS_PATH, mUseSwEncoderPkgs);
    }
    void doSwEncoderSetting() {
        if (!mUseSwEncoder) {
            return;
        }
        TaskRecord axr = mService.getMostRecentTask();
        ...
        // to ensure the current task has changed
        if (mSwEncoderCurrentTask.equals(pkgName)) {
            return;
        }
        mSwEncoderCurrentTask = pkgName;

        if (mUseSwEncoderPkgs.contains(pkgName)) {
            SystemProperties.set("mstar.camera.swenc","1");
            Slog.i("camera", "use swenc = " + pkgName);
        } else {
            SystemProperties.set("mstar.camera.swenc","0");
            Slog.i("camera", "use hwenc = " + pkgName);
        }
    }

    private boolean initCpuDvfsScaling() {
        return parsePackagesList("utf-8", CPU_DVFS_SCALING_PKGS_PATH, mCpuDvfsScalingPkgs);
    }

    void doCpuDvfsScaling() {
        if (mCpuDvfsScaling == false) return;
        // to ensure current task changed
        if (mService.getMostRecentTask() == null || mService.getMostRecentTask().affinity == null) {
            // should not happen
            return;
        }
        final String lastestTask = mService.getMostRecentTask().affinity;
        if (mCpuDvfsScalingCurrentTask.equals(lastestTask)) {
            return;
        }
        mCpuDvfsScalingCurrentTask = lastestTask;

        // to check target package name
        String action = "reset";
        if (mService.getRecentTasksSize() > 1) {
            // fetch package name from task
            int axIndex = mService.getRecentTasksSize()-1;
            int topxIndex = mService.getRecentTasksSize()-2;
            TaskRecord axr = mService.getMostRecentTask();
            TaskRecord topxr = mService.getMost2ndRecentTask();
            if (topxr == null || topxr.affinity == null) {
                // in case that some process has died
                return;
            }
            Slog.i(TAG, "TR" + axIndex + ") "+ axr.toString());
            Slog.i(TAG, "TR" + topxIndex + ") "+ topxr.toString());

            // action only if XOR is true that most recent two tasks are in allowed list
            if (mCpuDvfsScalingPkgs.contains(axr.affinity)) {
                action = "benchmark";
            }
        }

        // to nofity external service
        if (action == null) return;
        SystemProperties.set("mstar.cpu.dvfs.scaling", action);

        try {
            do_exec("stop cpudvfsscaling");
        } catch ( Exception e) {
            e.printStackTrace();
        }

        try {
            do_exec("start cpudvfsscaling");
        } catch ( Exception e) {
            e.printStackTrace();
        }
    }

    void doCpuDvfsScalingAgain() {
        if (false == "benchmark".equals(android.os.SystemProperties.get("mstar.cpu.dvfs.scaling"))) {
            return;
        }
        // for the case if STR resumed, system is running benchmark app
        try {
            do_exec("start cpudvfsscaling");
        } catch ( Exception e) {
            e.printStackTrace();
    ...
    private final boolean initBenchmarkAccelerate() {
        // So far we support only Antutu
        mBenchmarkAcceleratorPkgs.add("com.antutu.ABenchMark");
        mBenchmarkAcceleratorPkgs.add("com.antutu.benchmark.bench64");
        Slog.i(TAG, "accel87) adding allowed pkg=" + "com.antutu.ABenchMark");
        return true;
    }

    final void doBenchmarkAccelerate() {
        if (mBenchmarkAcceleratorEnabled == false) return;
        ...
        if (mService.getRecentTasksSize()>1) {
            ...
            // MStar Android Patch Begin
            String topPkg = axr.affinity.toString();
            topPkg = (topPkg.length()>SystemProperties.PROP_VALUE_MAX)?topPkg.substring(0, SystemProperties.PROP_VALUE_MAX):topPkg;
            SystemProperties.set("mstar.activity.recent", topPkg);
            // MStar Android Patch End
            Slog.i(TAG, "TR" + axIndex + ") "+ axr.toString());
            Slog.i(TAG, "TR" + topxIndex + ") "+ topxr.toString());

            // action only if XOR is true that most recent two tasks are in allowed list
            if (mBenchmarkAcceleratorPkgs.contains(topxr.affinity)) {
                // need no action between allowed packages
                if (!mBenchmarkAcceleratorPkgs.contains(axr.affinity)) {
                    action = "reset";
                }
            } else if (mBenchmarkAcceleratorPkgs.contains(axr.affinity)) {
                action = "benchmark";
            }
        }
        ...
        // to nofity external service
        if (action==null) return;
        SystemProperties.set("mstar.benchmark.accel87", action);
        Slog.i(TAG, "accel87=" + action);

        // ready to do benchmark accelerate
        try {
            do_exec("start accel87");
        } catch ( Exception e) {
            e.printStackTrace();
        }
    }

    private boolean initVideoTalk() {
        return parsePackagesList("utf-8", VIDEO_TALK_PKGS_PATH, mVideoTalkPkgs);
    }

    void doVideoTalkSetting() {
        if (!mVideoTalk) {
            return;
        }
        TaskRecord axr = mService.getMostRecentTask();
        ...
        // to ensure the current task has changed
        if (mVideoTalkCurrentTask.equals(pkgName)) {
            return;
        }
        mVideoTalkCurrentTask = pkgName;

        if (mVideoTalkPkgs.contains(pkgName)) {
            SystemProperties.set("debug.videotalk.lantency","1");
            Slog.i("VideoTalk", "enable lantency mode = " + pkgName);
        } else {
            SystemProperties.set("debug.videotalk.lantency","0");
            Slog.i("VideoTalk", "disable lantency mode = " + pkgName);
        }
    }
    // MStar Android Patch End
}
