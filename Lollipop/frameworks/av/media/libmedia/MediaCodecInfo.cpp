...
#define PROCESS_XTS_MEDIA "com.google.android.xts.media"
#define PROCESS_CTS_MEDIA "com.android.cts.media"
...
void MediaCodecInfo::getSupportedMimes(Vector<AString> *mimes) const {
    ...
    // MStar Android Patch Begin
    /* XTS Vp8CodecTest will launch more than one decoder simultaneously.
     * However, MStar HW decoder do not support this on some platform.
     * Hence, we don't let XTS choose OMX.MS.VP8.Decoder. */
    /* MS VP8 does not support adaptive playback yet, we bypass CTS test
       here */
    if (isFromSpecificProcess(PROCESS_XTS_MEDIA) ||
        isFromSpecificProcess(PROCESS_CTS_MEDIA)) {
        const char *codecName = getCodecName();
        if (!strcmp("OMX.MS.VP8.Decoder", codecName))
            return;
    }
    // MStar Android Patch End
