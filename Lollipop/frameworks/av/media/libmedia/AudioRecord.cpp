        ...
        const audio_attributes_t* pAttributes)
{
	...
	ALOGV("set()_1_inputSource:%d", inputSource);
	if (inputSource == AUDIO_SOURCE_CMCC_KARAOK_MIC) {
        inputSource = AUDIO_SOURCE_CAPTURE_DEVICE0;
    }else if(inputSource == AUDIO_SOURCE_CMCC_KARAOK_SPEAKER){
        inputSource = AUDIO_SOURCE_CAPTURE_DEVICE1;
    }
	ALOGV("set()_2_inputSource:%d", inputSource);
	// MStar Android Patch for CMCC En
