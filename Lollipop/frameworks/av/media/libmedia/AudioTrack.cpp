...
ssize_t AudioTrack::write(const void* buffer, size_t userSize, bool blocking)
{
    ...
    if (pDumpFile == NULL)
    {
        ...
        if ((property_get("dump.audio.before.af.on", propValue, NULL) > 0) && (atoi(propValue) == 1))
        {
            char fileName[128] = {0};
            snprintf(fileName, sizeof(fileName), "/data/audioTrack_dump_data_%u.pcm", gettid());
            pDumpFile = fopen(fileName, "ab+");
        }
    }
    // MStar Android Patch End
    while (userSize >= mFrameSize) {
        ...
        if (pDumpFile != NULL)
        {
            fwrite(buffer, 1, toWrite, pDumpFile);
        }
        // MStar Android Patch End
