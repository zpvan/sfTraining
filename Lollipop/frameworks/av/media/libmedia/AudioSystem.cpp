...
int AudioSystem::gAudioPortCallbackRefCount = 0;
...
void AudioSystem::setAudioPortCallback(sp<AudioPortCallback> callBack)
{
    ...
    // MStar Android Patch Begin
    if (callBack != 0) {
        gAudioPortCallbackRefCount++;
        gAudioPortCallback = callBack;
    } else {
        if (gAudioPortCallbackRefCount && (--gAudioPortCallbackRefCount == 0))
            gAudioPortCallback = callBack;
    }

    ALOGD("Setting global gAudioPortCallback %s refCnt: %d",
         (gAudioPortCallback == 0) ? "Off" : "On", gAudioPortCallbackRefCount);
    // MStar Android Patch End
}
