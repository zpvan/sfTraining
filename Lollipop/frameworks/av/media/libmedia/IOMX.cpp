...
#ifdef OPERATOR_PROJECT_CMCC
private:
    char pMimeType[OMX_MAX_STRINGNAME_SIZE];
#endif
...
public:
    BpOMX(const sp<IBinder> &impl)
        ...
        // MStar Android Patch Begin, for C2_CMCC
        #ifdef OPERATOR_PROJECT_CMCC
        memset(pMimeType, 0, OMX_MAX_STRINGNAME_SIZE);
        #endif
        // MStar Android Patch End, for C2_CMCC
    }
    ...
    virtual status_t getParameter(
        ...
        if (index == OMX_IndexParamPortDefinition) {
           snprintf(pMimeType, OMX_MAX_STRINGNAME_SIZE, "%s", reply.readCString());
        }
        #endif
        // MStar Android Patch End, for C2_CMCC
        status_t err = reply.readInt32();
        ...
        if (index == OMX_IndexParamPortDefinition) {
            ...
            if (defParams->eDomain == OMX_PortDomainAudio) {
                defParams->format.audio.cMIMEType = pMimeType;
            }
            else if (defParams->eDomain == OMX_PortDomainVideo) {
                defParams->format.video.cMIMEType = pMimeType;
            }
        }
        #endif
        // MStar Android Patch End, for C2_CMCC
    ...
    virtual status_t setParameter(
        ...
        if (index == OMX_IndexParamPortDefinition) {
            ...
            if (defParams->eDomain == OMX_PortDomainAudio) {
                mimeType = defParams->format.audio.cMIMEType;
            }
            if (defParams->eDomain == OMX_PortDomainVideo) {
                mimeType = defParams->format.video.cMIMEType;
            }
            data.writeCString(mimeType);
        }
        #endif
        // MStar Android Patch End, for C2_CMCC
            ...
            switch (code) {
                ...
                case SET_PARAMETER:
                    ...
                    if (index == OMX_IndexParamPortDefinition) {
                        OMX_PARAM_PORTDEFINITIONTYPE *defParams = (OMX_PARAM_PORTDEFINITIONTYPE *)params;
                        const char * mimeType = NULL;
                        mimeType = data.readCString();

                        if (defParams->eDomain == OMX_PortDomainAudio) {
                            defParams->format.audio.cMIMEType = const_cast<char *> (mimeType);
                        }
                        else if (defParams->eDomain == OMX_PortDomainVideo) {
                            defParams->format.video.cMIMEType = const_cast<char *> (mimeType);
                        }
                    }
                    #endif
                    // MStar Android Patch End, for C2_CMCC
