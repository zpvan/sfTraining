...
#define MULTI_CHANNEL_NUM                 6
#define AUDIO_PARAMETER_BYPASSMODE        "ByPass"
#define AUDIO_PARAMETER_MIXMODE           "MixInHAL"
...
// Integer part: Volume table with -1db per step, Gain setting = 12db - N * 1.0db (+12db ~ -114db)
// Fraction part: Volume table with -0.125 db per step
#define AUDIO_VOLUME_ARRAY_NUMBER 101
const unsigned short volume_curve[AUDIO_VOLUME_ARRAY_NUMBER] =
{
...
const StringToEnum sDeviceNameToEnumTable[] = {
    ...
    // For tunnel mode support
    STRING_TO_ENUM(AUDIO_DEVICE_OUT_DIRECT_PCM),
    STRING_TO_ENUM(AUDIO_DEVICE_OUT_RAW),
    STRING_TO_ENUM(AUDIO_DEVICE_OUT_MULTI_CHANNEL),
    // MStar Android Patch End
    ...
    // MStar Android Patch Begin
    STRING_TO_ENUM(AUDIO_DEVICE_IN_BT_MIC),
                                                         ...
                                                         const char *device_address)
{
    ...
    if (audio_is_output_device(device)) {
        ...
        case AUDIO_POLICY_DEVICE_STATE_AVAILABLE: {
            ...
            if (audio_is_a2dp_device(device)) {
                ALOGV("audio_is_a2dp_device(device) is true");
                ...
                audiomanager->enableMute(MSTAR_TVSPEAKER_MUTE);
            }
            // MStar Android Patch End
        ...
        //ALOGE("%s__ mOutputs.size:%d ", __FUNCTION__, mOutputs.size());
    ...
    // MStar Android Patch Begin
    // Volume up/down, mute function
    //ALOGE("%s__ state:%d, device:%x ", __FUNCTION__, state, device);
#if 1
    if (state == AUDIO_POLICY_DEVICE_STATE_UNAVAILABLE) {
        if (audio_is_a2dp_device(device)) {
             if (audiomanager == NULL) {
            ...
            //ALOGE("%s__ Unmute speaker!",__FUNCTION__);
            audiomanager->disableMute(MSTAR_TVSPEAKER_MUTE);
        }
    }
    // MStar Android Patch End
#endif
                                         ...
                                         audio_policy_forced_cfg_t config)
{
    ...
    case AUDIO_POLICY_FORCE_FOR_COMMUNICATION:
        if (config != AUDIO_POLICY_FORCE_SPEAKER && config != AUDIO_POLICY_FORCE_BT_SCO &&
            // MStar Android Patch Begin
            config != AUDIO_POLICY_FORCE_BT_MIC &&
            // MStar Android Patch End
    ...
    case AUDIO_POLICY_FORCE_FOR_RECORD:
        if (config != AUDIO_POLICY_FORCE_BT_SCO && config != AUDIO_POLICY_FORCE_WIRED_ACCESSORY &&
            // MStar Android Patch Begin
            config != AUDIO_POLICY_FORCE_BT_MIC && config != AUDIO_POLICY_FORCE_NONE &&
            // MStar Android Patch End
            config != AUDIO_POLICY_FORCE_NONE) {
                                              ...
                                              const audio_offload_info_t *offloadInfo)
{
    ...
    if (strategy == STRATEGY_MEDIA) {
        if ((format & AUDIO_FORMAT_MAIN_MASK) != AUDIO_FORMAT_PCM) {
            device = mAvailableOutputDevices.types() & AUDIO_DEVICE_OUT_RAW;
        }
        else if ((flags & AUDIO_OUTPUT_FLAG_HW_AV_SYNC) != 0) {
            device = mAvailableOutputDevices.types() & AUDIO_DEVICE_OUT_DIRECT_PCM;
        }
        else if ((device == AUDIO_DEVICE_OUT_SPEAKER)
                && (audio_channel_count_from_out_mask(channelMask) >= MULTI_CHANNEL_NUM)
                && (mAvailableOutputDevices.types() & AUDIO_DEVICE_OUT_MULTI_CHANNEL)) {
            // Multi-Channel not suppport following output flags, use deafult audio policy flow
            audio_output_flags_t ExcludeMask  = (audio_output_flags_t)
                                (AUDIO_OUTPUT_FLAG_FAST | AUDIO_OUTPUT_FLAG_DEEP_BUFFER |
                                AUDIO_OUTPUT_FLAG_COMPRESS_OFFLOAD | AUDIO_OUTPUT_FLAG_NON_BLOCKING |
                                AUDIO_OUTPUT_FLAG_HW_AV_SYNC);
            if ((flags & ExcludeMask) == 0) {
                device = AUDIO_DEVICE_OUT_MULTI_CHANNEL;
            }
        }
        // else go to regular flow
    }
    // MStar Android Patch End
        ...
        const audio_offload_info_t *offloadInfo)
{
    ...
    if (((flags & AUDIO_OUTPUT_FLAG_DIRECT) == 0) &&
            ...
            // MStar Android Patch Begin
            // Android original design can't accept multi-channel device as mixer thread
                                             ...
                                             audio_session_t session)
{
    ...
    if (outputDesc->mRefCount[stream] == 1) {
        ...
        // MStar Android Patch Begin
        // Two mm play support speaker and line out
        setStreamVolumeIndex(AUDIO_STREAM_MUSIC_SUB, mStreams[AUDIO_STREAM_MUSIC_SUB].mIndexMax, newDevice);
        // MStar Android Patch End
...
{
    AudioPolicyManager* apm = reinterpret_cast<AudioPolicyManager*>(input);
...
void AudioPolicyManager::reLoadHwModule()
{
    ...
    mBeaconMuted(false)
{
    ...
    sp<DeviceDescriptor> lineOutDevice = new DeviceDescriptor(String8(""), AUDIO_DEVICE_OUT_LINE_OUT);
    mAvailableOutputDevices.add(lineOutDevice);
    // For tunnel mode support
    sp<DeviceDescriptor> directDevice = new DeviceDescriptor(String8(""), AUDIO_DEVICE_OUT_DIRECT_PCM);
    mAvailableOutputDevices.add(directDevice);
    sp<DeviceDescriptor> rawDevice = new DeviceDescriptor(String8(""), AUDIO_DEVICE_OUT_RAW);
    mAvailableOutputDevices.add(rawDevice);
    sp<DeviceDescriptor> MultiChannelDevice = new DeviceDescriptor(String8(""), AUDIO_DEVICE_OUT_MULTI_CHANNEL);
    mAvailableOutputDevices.add(MultiChannelDevice);
    // MStar Android Patch End
    ...
    for (size_t i = 0; i < mHwModules.size(); i++) {
        ...
        for (size_t j = 0; j < mHwModules[i]->mOutputProfiles.size(); j++)
        {
            ...
            if ((outProfile->mFlags & AUDIO_OUTPUT_FLAG_DIRECT) != 0) {
                ALOGV("Profile with Direct Flag Size %d",outProfile->mSupportedDevices.size());
            ...
            if (status == NO_ERROR) {
                for (size_t k = 0; k  < inProfile->mSupportedDevices.size(); k++) {
                    ...
                    if ((index >= 0) && (mAvailableInputDevices[index]->mId == 0)) {
                        ...
                        // MStar Android Patch Begin
                        // To make the audio gain config available for TIF
                        mAvailableInputDevices[index]->importAudioPort(inProfile);
                        // MStar Android Patch End
                    }
                }
                mpClientInterface->closeInput(input);
    ...
    // MStar Android Patch Begin
    // Two mm play support speaker and line out
    initStreamVolume(AUDIO_STREAM_MUSIC_SUB, 0, 15);
    // MStar Android Patch End
...
void AudioPolicyManager::checkOutputForStrategy(routing_strategy strategy)
{
    ...
    if (!vectorsEqual(srcOutputs,dstOutputs)) {
        ...
        // MStar Android Patch Begin
        // Two mm play support speaker and line out
        if ((strategy == STRATEGY_MEDIA) || (strategy == STRATEGY_MEDIA_SUB)) {
        // MStar Android Patch End
    ...
    if (mA2dpSuspended) {
        if ((!isScoConnected ||
             ((mForceUse[AUDIO_POLICY_FORCE_FOR_COMMUNICATION] != AUDIO_POLICY_FORCE_BT_SCO) &&
              ...
              // MStar Android Patch Begin
              (mForceUse[AUDIO_POLICY_FORCE_FOR_COMMUNICATION] != AUDIO_POLICY_FORCE_BT_MIC) &&
              (mForceUse[AUDIO_POLICY_FORCE_FOR_RECORD] != AUDIO_POLICY_FORCE_BT_MIC) &&
              // MStar Android Patch End
...
audio_devices_t AudioPolicyManager::getDevicesForStream(audio_stream_type_t stream) {
    ...
    //ALOGE("%s__ devices:%x, strategy:%d ", __FUNCTION__, devices, strategy);
    return devices;
}
...
uint32_t AudioPolicyManager::getStrategyForAttr(const audio_attributes_t *attr) {
    ...
    // MStar Android Patch Begin
    // Two mm play support speaker and line out
    case AUDIO_USAGE_MEDIA_SUB:
        return (uint32_t) STRATEGY_MEDIA_SUB;
    // MStar Android Patch End
                                                             ...
                                                             bool fromCache)
{
    ...
    case STRATEGY_SONIFICATION_RESPECTFUL:
        ...
        // MStar Android Patch Begin
        // Two mm play support speaker and line out
        else if (isStreamActive(AUDIO_STREAM_MUSIC_SUB, SONIFICATION_RESPECTFUL_AFTER_MUSIC_DELAY)) {
    ...
    // MStar Android Patch End
        ...
        // MStar Android Patch Begin
        // Two mm play support speaker and line out
        if ((device2 == AUDIO_DEVICE_NONE) && (strategy == STRATEGY_MEDIA_SUB)) {
            device2 = availableOutputDeviceTypes & AUDIO_DEVICE_OUT_LINE_OUT;
        }
        // MStar Android Patch End
...
audio_devices_t AudioPolicyManager::getDeviceForInputSource(audio_source_t inputSource)
{
    ...
    case AUDIO_SOURCE_BLUETOOTH_MIC:
        if (mForceUse[AUDIO_POLICY_FORCE_FOR_RECORD] == AUDIO_POLICY_FORCE_BT_MIC &&
                availableDeviceTypes & AUDIO_DEVICE_IN_BT_MIC) {
                device = AUDIO_DEVICE_IN_BT_MIC;
        }
        break;
    case AUDIO_SOURCE_CAPTURE_DEVICE0:
        device = AUDIO_DEVICE_IN_CAPTURE_DEVICE0;
        break;
    case AUDIO_SOURCE_CAPTURE_DEVICE1:
    ...
    case AUDIO_SOURCE_HOTWORD:
        ...
        if (mForceUse[AUDIO_POLICY_FORCE_FOR_RECORD] == AUDIO_POLICY_FORCE_BT_MIC &&
            ...
            inputSource == AUDIO_SOURCE_MIC)) { // Broadcom input mic
            /* Only for VoiceRecognition and MIC audio source, check for BT_MIC */
            device = AUDIO_DEVICE_IN_BT_MIC;
            ALOGE("picked the Bcmbt MIC");
        } else if (inputSource == AUDIO_SOURCE_VOICE_RECOGNITION ||
            inputSource == AUDIO_SOURCE_MIC) { // Mediatek input mic
        ...
        int indexInUi)
{
    ...
    // MStar Android Patch Begin
    // compute volume to DB according to the mstar volume curve
    // apply mstar volume cuver if stream volume index range is 101
    if ((streamDesc.mIndexMax == (AUDIO_VOLUME_ARRAY_NUMBER - 1))
        && (streamDesc.mIndexMin == 0)) {
        float volDbInt = 0.0f;
        float volDbFrac = 0.0f;

        volDbInt = ((volume_curve[indexInUi] >> 8) - 0x0C) * (-1.0f);
        volDbFrac = (volume_curve[indexInUi] & 0xFF) * (-0.125f);
        return exp( (volDbInt + volDbFrac) * 0.115129f); // exp( dB * ln(10) / 20 )
    }
    // MStar Android Patch End
                                            ...
                                            audio_devices_t device)
{
    ...
    if ((device & (AUDIO_DEVICE_OUT_BLUETOOTH_A2DP |
        ...
        // MStar Android Patch Begin
        // Two mm play support speaker and line out
        if (isStreamActive(AUDIO_STREAM_MUSIC_SUB, SONIFICATION_HEADSET_MUSIC_DELAY) ||
                mLimitRingtoneVolume) {
            audio_devices_t musicDevice = getDeviceForStrategy(STRATEGY_MEDIA_SUB, true /*fromCache*/);
            float musicVol = computeVolume(AUDIO_STREAM_MUSIC_SUB,
                               mStreams[AUDIO_STREAM_MUSIC_SUB].getVolumeIndex(musicDevice),
                               output,
                               musicDevice);
            float minVol = (musicVol > SONIFICATION_HEADSET_VOLUME_MIN) ?
                                musicVol : SONIFICATION_HEADSET_VOLUME_MIN;
                                                   ...
                                                   bool force)
{
    ...
    //MStar patch start
#if 0
    ///< we set the Input source (main channel) volume when user do set volume except KTV.(mic volume should be control by ktv app)
    ///< and current view if desktop.(preview small window)
    ///< and we not need set main channel vol under input source MM
    if (stream == AUDIO_STREAM_MUSIC) {
        char prop[PROPERTY_VALUE_MAX] = {0};
        property_get("sys.boot_completed", prop, "0");
        if (1 == atoi(prop)) {
            int source = TvManager::getCurrentInputSource();
            if ((source > 0) && (source < 42)) {

                sp<AudioManager> audiomanager = NULL;
                audiomanager = AudioManager::connect();
                if (audiomanager == NULL) {
                    ALOGE("Fail to connect to audio manager service %d.", __LINE__);
                    return BAD_VALUE;
                }
                ...
                index = 0.5f + index * (100.0f/mStreams[stream].mIndexMax);
                //ALOGD("%s__ stream:%d, source:%d, device:%x, index:%d "
                //       ,__FUNCTION__, stream, source, device, index);
                if (device == AUDIO_DEVICE_OUT_SPEAKER) {
                    audiomanager->setKtvMixModeVolume(0, index, 0);
                } else if (device == AUDIO_DEVICE_OUT_BLUETOOTH_A2DP) {
                    audiomanager->setKtvMixModeVolume(1, index, 0);
                }
    ...
    if (stream == AUDIO_STREAM_MUSIC) {
	    index = 0.5f + index * (100.0f/mStreams[stream].mIndexMax);
        if ((device & AUDIO_DEVICE_OUT_SPEAKER) || (device & AUDIO_DEVICE_OUT_MULTI_CHANNEL)) {
            ALOGVV("checkAndSetVolume() AUDIO_DEVICE_OUT_SPEAKER ");
            mpClientInterface->setParameters(0, String8::format("spk_volume=%d", index));
            volume = 1.0f;
        } else if (device & AUDIO_DEVICE_OUT_BLUETOOTH_A2DP) {
            ALOGE("checkAndSetVolume() AUDIO_DEVICE_OUT_BLUETOOTH_A2DP bluetooth_volume=%d", index);
            mpClientInterface->setParameters(0, String8::format("bluetooth_volume=%d", index));
            volume = 1.0f;
        }
    }
    // MStar Android Patch End
