        ...
        audio_io_handle_t output)
{
    ...
    // MStar Android Patch Begin
    // This is to patch TV volume control issue after AN6.0. It can be removed once TIF framework is applied in every TV APK
    if (stream == AUDIO_STREAM_MUSIC) {
        AutoMutex lock(mHardwareLock);
        AudioHwDevice *dev = mPrimaryHardwareDev;

        mHardwareStatus = AUDIO_HW_SET_MASTER_VOLUME;
        if (dev && dev->canSetMasterVolume()) {
            dev->hwDevice()->set_master_volume(dev->hwDevice(), value);
        }
        mHardwareStatus = AUDIO_HW_IDLE;
    }
