...
ssize_t AudioFlinger::PlaybackThread::threadLoop_write()
{
    ...
    if (mNormalSink != 0) {
        ...
        // MStar Android Patch Begin
#if DEBUG_PCM_ENABLE
        static const char fileName[128] =  "/data/OutputData.pcm";
        FILE *pdumpFile = fopen(fileName, "ab+");

        LOG_ALWAYS_FATAL_IF((pdumpFile == NULL), "OutputDataFile Open Failed");
        fwrite((char *)mSinkBuffer + offset, 1, mBytesRemaining, pdumpFile);
        fflush(pdumpFile);
        fclose(pdumpFile);
#endif
        // MStar Android Patch End
...
void AudioFlinger::MixerThread::threadLoop_sleepTime()
{
    ...
    // Trigger a2dp thread outwrite firstly when a2dp connected
        ...
        if (outDevice() & AUDIO_DEVICE_OUT_ALL_A2DP) {
            ALOGD("MIXER_IDLE out_write to support A2DP output:outdevice is %d", outDevice());
            sleepTime = 0;
        }
    }
    // MStar Android Patch End
    ...
    for (size_t i=0 ; i<count ; i++) {
                ...
                !track->isPaused() && !track->isTerminated())
        {
            ...
            if (track->isPausing() || mStreamTypes[track->streamType()].mute) {
                ...
                // MStar Android Patch Begin
#if DEBUG_PCM_ENABLE
                ALOGI("VOL: master[%f]>>>stream[%f] of type[%d]>>>vlr[%d]",masterVolume,typeVolume, track->streamType(),vlr);
#endif
                // MStar Android Patch End
...
track_is_ready: ;
    ...
    // MStar Android Patch Begin
    // sink or mix buffer must be cleared if at least one tracks are connected to an
    ...
    , mFastTrackAvail(false)
{
    ...
    case FastCapture_Static:
        ...
        // MStar Android Patch Begin
        // Check if the HAL is prepared to support Fast Capture. If not, turn it off.
        if (initFastCapture) {
            char *defeat =
                input->stream->common.get_parameters((const struct audio_stream *)&input->stream->common,
                                                     "FastCaptureDefeat");
            if (strcmp(defeat,"true") == 0) {
                ALOGE("FastCaptureDefeat: %s", defeat);
