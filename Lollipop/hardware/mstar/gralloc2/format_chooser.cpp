...
uint64_t gralloc_select_format(int req_format, int usage, int buffer_size)
{
    ...
    if(req_format == HAL_PIXEL_FORMAT_IMPLEMENTATION_DEFINED)
    {
        ...
        /* MStar: Camera uses 422_I */
#ifdef MSTAR_GRALLOC
        if( usage & GRALLOC_USAGE_HW_CAMERA_MASK )
        {
            new_format = HAL_PIXEL_FORMAT_YCbCr_422_I;
        }
#endif
    }
    ...
    // MStar: do not select AFBC formats if AFBC usage is not ON
#if defined(MSTAR_GRALLOC_AFBC) && MALI_AFBC_GRALLOC
    if( !(usage & GRALLOC_USAGE_AFBC_ON) )
    {
        return new_format;
    }
#else
