...
#ifndef MSTAR_GRALLOC_NO_VSYNC
#include "gralloc_vsync.h"
#endif

#ifdef MSTAR_FAKE_FBDEV
#include <cutils/properties.h> /* for PROPERTY_VALUE_MAX */
#include "fbdev.h"
#endif
...
#ifdef MSTAR_FAKE_FBDEV
int get_framebuffer_format(void)
{
    char property[PROPERTY_VALUE_MAX] = {0};
    int format = 0;

    if (property_get("mstar.framebuffer.format", property, 0))
    {
        format = atoi(property);
    }
    ...
    case 2: /* ARGB8888 */
        return HAL_PIXEL_FORMAT_RGBA_8888; /* Use RGBA instead of BGRA coz it still has problem */
...
int init_frame_buffer_locked(struct private_module_t* module)
{
    ...
    {
#ifdef MSTAR_FAKE_FBDEV
        info.width  = ((info.xres * 25.4f)/density + 0.5f);
        info.height = ((info.yres * 25.4f)/density + 0.5f);
#else
...
#ifdef MSTAR_FAKE_FBDEV
    if ((info.reserved[0] > info.xres) || (info.reserved[1] > info.yres))
    {
        xdpi = (info.reserved[0] * 25.4f) / (int)((info.reserved[0] * 25.4f) / density + 0.5f);
        ydpi = (info.reserved[1] * 25.4f) / (int)((info.reserved[1] * 25.4f) / density + 0.5f);
        fps  = (1000000000000000LLU /
                (
                 uint64_t(info.upper_margin + info.lower_margin + info.reserved[1])
                          * (info.left_margin + info.right_margin + info.reserved[0])
                          * info.pixclock
                          )) / 1000.0f;
    }

    ALOGI("info.reserved[0] = %d, info.reserved[1] = %d\n",info.reserved[0], info.reserved[1]);
#endif
    ...
    // MStar: it's not necessary to clear FB. MMAP_GOP_FB has been cleared using GE when hwcomposer module init
...
#ifdef MSTAR_FAKE_FBDEV
    module->framebuffer = new private_handle_t(private_handle_t::PRIV_FLAGS_FRAMEBUFFER, GRALLOC_USAGE_HW_FB, fbSize, vaddr, 0, finfo.smem_start, 0);
#else
...
#endif
#else
    const_cast<int &>(dev->format) = get_framebuffer_format();
    const_cast<int &>(dev->osdWidth) = m->info.reserved[0];
    const_cast<int &>(dev->osdHeight) = m->info.reserved[1];
#endif
