...
#ifndef HWCOMPOSEROSD_H_
#define HWCOMPOSEROSD_H_
...
typedef std::vector<std::shared_ptr<HWWindowInfo>> HWWindowInfo_data;

class HWComposerOSD {
public:

    HWComposerOSD(HWCDisplayDevice& display);
    ~HWComposerOSD();
    hwc2_error_t present(std::vector<std::shared_ptr<HWCLayer>>& layers, int32_t* outRetireFence);
    uint32_t getDefaultGopNumber() {return mHwwininfo[0]->gop;}
    uint32_t getGopDest();
    bool checkGopTimingChanged();
    bool isGopSupportAFBC(int index);
    std::string dump() const;
private:
    void init();
    void osdDisplayWorkThread();
    ...
    void setStbResolutionState();
    void initXcGfx();
    void initCmdqAndMload();
    void clearGfxFbBuffer();
    hwc2_error_t display(HWWindowInfo_data& inHWwindowinfo);
    HWCDisplayDevice& mDisplay;
    std::mutex mMutex;
    std::condition_variable mCv;
    std::queue<HWWindowInfo_data> mDisplayQueue;
    std::unique_ptr<HWComposerGop> mGopDevice;
    HWWindowInfo_data mHwwininfo;
    HWWindowInfo_data mlastHWWindowInfo;
};

#endif /* HWCOMPOSERVIDEO_H_ */
