...
class HWComposerCursorDev;

class HWCCursor {
public:

    HWCCursor(HWCDisplayDevice& display);
    ~HWCCursor();
    hwc2_error_t present(std::vector<std::shared_ptr<HWCLayer>>& layers, int32_t* outRetireFence);

    hwc2_error_t redrawFb(int srcX, int srcY, int srcWidth, int srcHeight, int stride,int dstWidth, int dstHeight);
    hwc2_error_t setCursorPositionAsync(int32_t x_pos, int32_t y_pos);
    bool checkCursorFbNeedRedraw(std::vector<std::shared_ptr<HWCLayer>>& layers);
...
#endif
private:
    int init(int32_t devNo);
    int deinit(int32_t devNo);
    hwc2_error_t display();
    void cursorDisplayWorkThread(int devNo);
    void commitTransaction(int devNo);
    HWCDisplayDevice& mDisplay;
    std::vector<std::mutex> mMutex;
    std::vector<std::mutex> mExitMutex;
    std::vector<std::condition_variable> mCv;
    std::vector<std::queue<std::shared_ptr<HWCursorInfo>>> mDisplayQueue;
    std::vector<MS_BOOL> bThreadHasExit;
    int32_t mCursorLayerCount;
    std::vector<std::shared_ptr<HWCursorInfo>> mHWCursorInfo;
    std::vector<std::shared_ptr<HWCursorInfo>> mLastHWCursorInfo;
    std::vector<uintptr_t> mCursorPhyAddr;
    std::vector<uintptr_t> mCursorVaddr;
    std::unique_ptr<HWComposerCursorDev> mCursorDevice;
};
