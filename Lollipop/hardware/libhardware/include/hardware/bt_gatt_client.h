...
/** Triggered when a device RSSI passes a given threshold */
typedef void (*monitor_rssi_callback)(int client_if, bt_bdaddr_t* bda, int rssi);

/** Burst data transfer callback */
typedef void (*burst_data_callback)(int client_if, const bt_bdaddr_t* bda, int len, const char *data);
...
typedef struct {
    ...
    // MStar Android Patch Begin
    burst_data_callback                 burst_data_cb;
    // MStar Android Patch End
...
typedef struct {
    ...
    // MStar Android Patch Begin
    /** Register to receive burst data */
    bt_status_t (*burst_data_receive)(int client_if);
    // MStar Android Patch End
