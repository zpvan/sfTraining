...
typedef struct hwc_composer_device_1 {
    ...
    void (*captureScreenWithClip)(struct hwc_composer_device_1* dev,size_t numData, hwc_capture_data_t* capturedata, uintptr_t* addr, int* width, int* height, int withOsd);
    void (*captureScreen)(struct hwc_composer_device_1* dev, uintptr_t* addr, int* width, int* height, int withOsd);
    int (*releaseCaptureScreenMemory)(struct hwc_composer_device_1* dev);
    // MStar Android Patch Begin
    ...
    int (*getMirrorMode)(struct hwc_composer_device_1* dev, int* HMirror, int* VMirror);
    int (*isAFBCFramebuffer)(struct hwc_composer_device_1* dev);
    int (*bRGBColorSpace)(struct hwc_composer_device_1* dev);
    //for game cursor
    int (*setGameCursorShow)(struct  hwc_composer_device_1* dev);
    int (*setGameCursorHide)(struct  hwc_composer_device_1* dev);
    int (*setGameCursorAlpha)(struct  hwc_composer_device_1* dev, float alpha);
    int (*redrawGameCursorFb)(struct  hwc_composer_device_1* dev, int srcX, int srcY, int srcWidth, int srcHeight, int stride,int dstWidth, int dstHeight);
    int (*setGameCursorPosition)(struct hwc_composer_device_1 *dev, float x_pos, float y_pos);
    int (*doGameCursorTransaction)(struct  hwc_composer_device_1* dev);
    int (*getGameCursorFbVaddr)(struct  hwc_composer_device_1* dev, intptr_t* vaddr);
    int (*initGameCursor)(struct  hwc_composer_device_1* dev);
    //for hwcursor
    int (*redrawHwCursorFb)(struct  hwc_composer_device_1* dev, int srcX, int srcY, int srcWidth, int srcHeight, int stride,int dstWidth, int dstHeight);
    // MStar Android Patch End
