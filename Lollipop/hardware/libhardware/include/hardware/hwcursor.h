...
typedef struct hwcursor_fix_info {
    uintptr_t hwCursorAddr;
    uintptr_t hwCursorVaddr;
    int hwCursorWidth;
    int hwCursorHeight;
    int hwCursorStride;
    float matrix[4];
    uintptr_t hwCursorDstAddr[HWCURSOR_DSTFB_NUM];
...
typedef struct hwcursor_overscan_info {
    int left;
    int top;
    int right;
    int bottom;
} hwcursor_overscan_info;
...
typedef struct hwcursor_device_t {
    ...
    struct hwcursor_overscan_info hwOverscanInfo;
    ...
    int (*hwCursorOverscanChange)(struct hwcursor_device_t* window,int left, int top, int right, int bottom);
