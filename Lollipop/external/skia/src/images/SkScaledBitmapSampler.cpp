...
// MStar Android Patch Begin
bool SkScaledBitmapSampler::nextSaveToYpos(const uint8_t* SK_RESTRICT src, int32_t toYPos) {
    SkASSERT((unsigned)fCurrY < (unsigned)fScaledHeight);
    fCurrY = toYPos;
    char* pDestRow = fDstRow + fDstRowBytes * toYPos;
    bool hadAlpha = fRowProc(pDestRow, src + fX0 * fSrcPixelSize, fScaledWidth,
                             fDX * fSrcPixelSize, fCurrY, fCTable);
    return hadAlpha;
}

bool SkScaledBitmapSampler::nextWithoutStep(const uint8_t* SK_RESTRICT src) {
    SkASSERT((unsigned)fCurrY < (unsigned)fScaledHeight);
